...
public class ExceptionControllerAdvice {
    ...
    public ResponseEntity<ExceptionDto> handleConstraintViolationException(ConstraintViolationException ex) {
        var details = ex.getConstraintViolations().stream().collect(groupingBy(
            v -> StringUtils.substringAfterLast(v.getPropertyPath().toString(), '.'),
            Collectors.mapping(ConstraintViolation::getMessage, Collectors.joining("; "))
        ));
        return buildExceptionDto(HttpStatus.BAD_REQUEST, "Validation error", details);
    }
